...
import ConfigBuilderCreateNewModal from "./misc/ConfigBuilderCreateNewModal";
import {Typography} from "@mui/material";
import ConfigViewButtonGroup from "./misc/ConfigViewButtonGroup";
...
export default function ConfigBuilderPage() {
    ...
    const buildFormOptions = (config) => {
        ...
        let systemTypes = [];
        if (schemaData.systems) {
            // available schema types for systems
            const systems = Object.keys(schemaData.systems);
            systemTypes = systems.map((item) => {
        ...
        let moduleTypes = [];
        if (schemaData.modules) {
            // available schema types for modules
            const modules = Object.keys(schemaData.modules);
            moduleTypes = modules.map((item) => {
                return {label: item, value: item}
            });
    ...
    const doesSchemaExist = (isModule, systemType) => {
        return (isModule) ? moduleSchemas.includes(systemType) : systemSchemas.includes(systemType);
    }
        ...
        <div className='builder-content'>
            ...
            <Box className='side-menu'>
                <div className='search'>
                    ...
                    <div>Config Builder: <span style={{ textDecoration: 'underline', cursor: 'pointer', fontSize: '0.9em'}}>3.6.2024</span></div>
                </div>
                        ...
                        {modules.map((item, index) => (
                            <div key={`module-item-${index}`} className={`module-link ${item.id === selectedId ? "selected" : ""}`} onClick={() => handleMenuItemClick(item.id, 0) }>
                                <Stack direction="row" sx={{alignItems:"center", position: "relative"}}>
                                    ...
                                    {/*  DOES SCHEMA EXIST?  */}
                                    {/*{moduleSchemas.includes(item.type) ?*/}
                                    {renderModuleName(item)}
                                    {/*{doesSchemaExist(true, item.type) ?*/}
